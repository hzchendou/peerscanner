...
public class VarInt {
    ...
    public VarInt(byte[] buf, int offset) {
        ...
        if (first < 253) {
            ...
            value = TypeUtils.readUint16(buf, offset + 1);
            originallyEncodedSize = 3; // 1 marker + 2 data bytes (16 bits)
        } else if (first == 254) {
            value = TypeUtils.readUint32(buf, offset + 1);
            originallyEncodedSize = 5; // 1 marker + 4 data bytes (32 bits)
        } else {
            value = TypeUtils.readInt64(buf, offset + 1);
    ...
    public byte[] encode() {
        ...
        switch (sizeOf(value)) {
            ...
            case 3:
                bytes = new byte[3];
                bytes[0] = (byte) 253;
                TypeUtils.uint16ToByteArrayLE((int) value, bytes, 1);
                return bytes;
            case 5:
                bytes = new byte[5];
                bytes[0] = (byte) 254;
                TypeUtils.uint32ToByteArrayLE(value, bytes, 1);
                return bytes;
            default:
                bytes = new byte[9];
                bytes[0] = (byte) 255;
                TypeUtils.int64ToByteArrayLE(value, bytes, 1);
                return bytes;
        }
    }
}
